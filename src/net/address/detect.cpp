...
AddressType sfap::net::detect_address_type( std::string_view address ) noexcept {
    ...
    if ( std::regex_match( address.data(), address.data() + address.size(), ipv4_regex ) ) return AddressType::IPV4;
    else if ( std::regex_match( address.data(), address.data() + address.size(), hostname_regex ) ) return AddressType::HOSTNAME;
    else if ( std::regex_match( address.data(), address.data() + address.size(), ipv6_regex ) ) return AddressType::IPV6;
